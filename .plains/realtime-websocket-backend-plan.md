...
- All channels/messages live in React state, seeded from `seedChannels()`
...
- **Socket.IO** (server + client) — gives rooms (one per channel/DM), automatic
...
- **Persistence:** phase in — start with an authoritative **in-memory store** on
  the server (seeded from `seedChannels()`), then move to **SQLite**
...
**Client → server**
- `channel:join { channelId }` → server replies with history
- `channel:leave { channelId }`
- `message:send { channelId, text, clientId }`
- `thread:reply { channelId, parentId, text, clientId }`
- `reaction:toggle { channelId, msgId, emoji }`
...
- `reaction:updated { channelId, msgId, reactions }`
...
- **B — Server as source of truth (messages):** server-side store seeded from
  chat-data, room-per-channel; `channel:join` returns history; `message:send`
...
- **C — Threads, reactions, DMs over the wire:** broadcast/merge thread replies
  ...
  broadcasts `channel:created`) — integrates the existing Compose feature.
...
- **E — Persistence:** swap in SQLite for channels/messages/reactions with
  migrations + seed; history pagination (load older on scroll).
- **F — Auth & scale (later):** real identity (session/JWT) in the handshake +
  channel authorization (private channels, DMs); Redis adapter for multi-node
...
- Two browser sessions in the same channel see each other's messages within ~1s.
